...
``` r
wheel_sheet = 
  read_excel("./data./Trash-Wheel-Collection-Totals-8-6-19.xlsx", range = "A2:N408", sheet = "Mr. Trash Wheel") %>%
  na.omit(dumpster) %>%
  janitor::clean_names() %>%
...
In MR. Trash Wheel dataset, there are 344 observations and 14 columns.
Dumpster is the count of dump time.

The precipitation dataset contains 18 observations and 3 columns,
including 12 months in 2018 and 6 months in 2019.
